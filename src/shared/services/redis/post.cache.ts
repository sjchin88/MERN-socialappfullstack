...
export class PostCache extends BaseCache {
  ...
  /**
   * Get all posts from the cache associated with the given key from start to end
   * @param key
   * @param start set to 0 for all posts
   * @param end  set to -1 for all posts
   * @returns
   */
  public async getPostsFromCache(key: string, start: number, end: number): Promise<IPostDocument[]> {
    try {
      ...
      //get the set values in reverse order from start to end
      //if start(0) to end (-1) will return everything
      ...
      //Cast the replies from multi.exec() as PostCacheMultiType which we defined earlier
  ...
  /**
  * Get total number of Posts in the cache
  ...
  */
  ...
  /**
   * Get total number of Posts in the cache for a single user
   * @param uId
   * @returns
   */
  ...
  /**
   * Delete the post based on the key
   * @param key
   * @param currentUserId
   */
