...
export class UserCache extends BaseCache {
  ...
  public async saveUserToCache(key: string, userUId: string, createdUser: IUserDocument): Promise<void> {
    ...
    /**
     * Destructure the properties from the createdUser
     */
    const {
      _id,
      uId,
  ...
  public async getUsersFromCache(start: number, end: number, excludedUserKey: string): Promise<IUserDocument[]> {
    try {
      ...
      //get the set values in reverse order from start to end, get the values from the sorted set first
      ...
      // based on values from ZRANGE, get the user info one by one
      for (const key of response) {
        if(key !== excludedUserKey) {
          //Retrieve all hashset using multi.HGETALL
